...
$cpDashboardHref = static function (string $cp) use ($dateFrom, $dateTo): string {
    ...
    return 'costs.php?' . http_build_query($q);
};
...
$deferEndpoint = 'defer_costs.php';
$loadCharts = true;
$deferredStatusText = 'Chargement des coûts…';

$dashboardOgBase = monitor_dashboard_public_base($cfg);
$dashboardOgPage = $dashboardOgBase . '/' . basename((string) ($_SERVER['SCRIPT_NAME'] ?? 'costs.php'));
