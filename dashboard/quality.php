...
$cpDashboardHref = static function (string $cp) use ($dateFrom, $dateTo): string {
    ...
    return 'quality.php?' . http_build_query($q);
};
...
$loadCharts = false;
$deferredStatusText = 'Chargement de la qualité de classification…';

$dashboardOgBase = monitor_dashboard_public_base($cfg);
$dashboardOgPage = $dashboardOgBase . '/' . basename((string) ($_SERVER['SCRIPT_NAME'] ?? 'quality.php'));
